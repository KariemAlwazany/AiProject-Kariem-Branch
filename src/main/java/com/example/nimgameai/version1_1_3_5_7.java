...
public class version1_1_3_5_7 implements Initializable {
    ...
    private List<List<ImageView>> rows;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Collections.addAll(values, "1, 3, 5, 7");
        version1ComboBox1.getItems().addAll(values);
        rows = new ArrayList<>();
        int[] sticksPerRow = {1, 3, 5, 7};
    ...
    private int calculateBestMove(double result, int numPossibleMoves) {
        double weightForResult = 1.0;
        double weightForNumPossibleMoves = 0.5;

        double estimatedValue = weightForResult * result +
                weightForNumPossibleMoves * numPossibleMoves;


        int moveIndex = (int) Math.round(estimatedValue * (numPossibleMoves - 1));
    ...
    private void removeSticks(int moveIndex) {
        ...
        if (version1Hard1.isSelected()) {
                if (version1ComboBox1.getValue().equalsIgnoreCase("1, 3, 5, 7")) {
                    ...
                    if (version1Ai.isSelected()) {
                        ...
                        int moveIndex = calculateBestMove(result, root.children.size());

                        // Remove the sticks based on the calculated move
                        removeSticks(moveIndex);
                    } else if (version1Me.isSelected()) {
